...
void SymbolicState::buildStorage(set<ContractDefinition const*> const& _contracts)
{
	...
	for (auto contract: _contracts)
	{
		...
		// TODO z3 doesn't like empty tuples, so if the contract has 0
		// state vars we can't put it there. But then we should also check
		// in the other functions that the tuple access to the storage of
		// that contract doesn't break.
